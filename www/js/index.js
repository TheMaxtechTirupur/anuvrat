        ...
        $(document).on('click', '#submit', function() { // catch the form's submit event
            if($('#username').val().length > 0 && $('#password').val().length > 0){ 
                ...
                //action is functionality we want to call and outputJSON is our data
                    ...
                    //$.ajax({url: 'http://192.168.0.101:80/anuvratvidhyalaya.in/ios/auth.php',
                    $.ajax({url: 'http://anuvratvidhyalaya.in/ios/auth.php',
...
// ---Bus Chart button click event--- // 
            ...
            if($('#usernamee').val().length > 0 && $('#passworde').val().length > 0){
                    ...
                    $.ajax({url: 'http://anuvratvidhyalaya.in/ios/buschart.php',
                    //$.ajax({url: 'http://localhost/anuvratvidhyalaya.in/ios/buschart.php',
                    data: {action : 'authorization', formData : $('#check-homework').serialize()},
                    type: 'post', 
                    async: 'true', 
                    dataType: 'json',
